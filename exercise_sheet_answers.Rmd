...
$Y \times p$ is simply p because Y must sum to 1 (p + (1-p) = 1). Therefore:
...
$$\
\
The expectation of the parameter ($E(θ)$) is $E(p^2) =  p^2$/
There is a clear difference between the expectation of the parameter and the estimator:\
\
$$
\frac{p(1-p)}{n} + p^2 \not= p^2
$$\
