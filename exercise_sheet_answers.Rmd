...
\
This could repeat n times where n can denotes any integer (theoretically): $((1 - α)\times (1-β))^n$ meaning the proability will be a summation of all the rounds where n = 1-infinity $(\sum_{n=1}^{\infty})$

\
Eventual success from Alice (given that she throws first) therefore gives: $P(X = n) =  \sum_{n=1}^{\infty} ((1 - α)\times (1-β))^{n-1} \timesα$
...
Instead there must be a final $1-β$ before Alice is victorious. Thus,

$$
\Large 
P2 = \sum_{n=1}^{\infty} ((1-β) \times (1-α))^{n-1} \times (1-β) \times α 
$$
...
The tins' weight follows a normal distribution $μ = 426, σ = 21$.

The variance (Var) is therefore $441$ because var$= σ^2$.
