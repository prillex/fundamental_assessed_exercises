...
for(i in 1:X1){  # Sampling everyone
  ...
  if (X1_trials[i] == 1) {  # If a successes
    no._X1_successes <- no._X1_successes + 1  # Add 1 to no. successes
  }
}
...
for(i in 1:X2){  # Sampling everyone
  X2_trials[i] <- sample(results, 
                      size = 1, # 1 Bernoulli per person
                      replace = F,  # Only one decision per person (all people must be sampled)
                      prob = X2_probs)  # Pre-specified probabilities
  if (X2_trials[i] == 1) {  # If a success
    no._X2_successes <- no._X2_successes + 1  # Add 1 to no. successes
  }
}
...
- I have used GenAI to understand how Latex code can be integrated with Rmarkdown.
