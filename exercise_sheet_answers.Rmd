...
The equation $\sum_{n=0}^{\infty}n(n-1)r^{n-2}$ has no common ratio i.e. there are extra terms: $n(n-1)$ - a polynomial.This means it is not part of the geometric series. However, I can but we can use properties of the geometric series to work out the sum of the latter. I can do so by altering the geomtric series to take the same form of the equation of interest.
...
I have the variance and expected no. samples until success but i also need the expectation of n^2 becuase $n(n−1)=n^2−n$. Therefore to calculate the expectation of $n^2$: 
...
The random variables follow Bernoulli distributions because its success/failure (outcomes sum to 1), is independent and n is known. It is not repeated n times for each sample $(i)$. If this were the case then the distribution would become binomial.
...
\right.
$$
Each random variables can be denoted as $X_i^{(1)} \sim \text{Bernoulli}(p) \text{ for } i = 1...,175$.
\
...
\right.
$$
Again, each random variables can be denoted as $X_i^{(2)} \sim \text{Bernoulli}(p) \text{ for } i = 1...,186$.
\


#### Expressing the number of B supporters at the end of the first day:

This encompasses repeating the trial n-times, thus making both $X_i^{(1)}$ and $X_i^{(2)}$  follow a Binomial distribution.
CONTINUE HERE
