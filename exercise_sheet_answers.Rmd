...
\Large
ȳ= \frac{\sum_{i=1}^{182}yi} {n} = \frac{60}{182}=0.330
...
$$\
\
Chain rule: $\left( \sum_{i=1}^n y_i \right)$ = constant, f(x) = ln, g(x) = lambda

$$
\frac{d }{d\lambda}(\left( \sum_{i=1}^n y_i \right) \ln \lambda) = \frac{\sum_{i=1}^n y_i}{\lambda}
$$\
\
Chain rule: $-n$ = constant, $f(x) = ln \text{ and } g(x) = 1-e^{-\lambda}$:\
...
\
\## Question 5c To plot the \$ -n + \frac{\sum_{i=1}^n y_i}{\lambda}-\frac{ne^{-\lambda}}{1-e^{-\lambda}}\$\
...
The output is:\
\
![image](images\ozone_relationships.png)\
...
This gives:\
\
![image](images\diagnostic_plots.png)\
\
The diagnostic plot give a host of insights into whether the model assumptions have been violated. Firstly, the 'Residuals vs Fitted' values reveal that there is a level of heteroscedascity in the data. It shows that the spread of residuals across differnt levels of the fitted values is inconsistent. Therfore the assumption that they are even (the assumption of homoscedacity) has been vioalted. In particular, there appears to be a more erratic spread of residuals at the extremes of the fitted values. A similar interpretation can be made for the 'Scale-Location' plot.
...
In conclusion, the model's reliability is limited in some respect. Namely, the lack of homoscedascity and nomrality in the residuals limits the validity of inferring the results of the model. \
\
\
...
\
![image](images\coefficients_comparison.png)\
\
...
\
\
![image](images\7c_diagnostic_plots.png)\
\

In comparison to the former models diagnostic plot, this 'Residuals vs Fitted' plot highlights a huge amelioration in the extent of heteroscedescity in the data. Now ther residuals are much more evenly distributed around the flat x axis line. This means that the variance of the residuals is more consitent and that the assumption of homoscedascity has not been vioalted to the same extent as before. Furthermore, the linearity of the residuals is much better as can be seen in the Q-Q plot; this means that the residuals follow the normal  distribution assumed of linear models. While I have not used statistical tests such as the Bartletts test for homoscedacity and the Shaprio Wilks for normality to test whether assumptions have been met, I can at least confirm through visual assessment of the diagnostic plot that the transformations have made the data in the second model more approporaite for a linear model.
